...
export const ClinicalGuidelines = ({ darkMode = false }) => {
    ...
    <div className={`border rounded-lg p-4 hover:shadow-md transition-shadow ${
      ...
      <div className="flex justify-between items-start gap-4">
        <div className="flex-1">
          ...
          <div className={`flex flex-wrap gap-3 mb-3 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          {guideline.keyRecommendations && guideline.keyRecommendations.length > 0 && (
            <div className="mb-3">
              <p className={`text-sm font-medium mb-1 ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Key Recommendations:</p>
              <ul className="list-disc ml-5 space-y-1">
                {guideline.keyRecommendations.slice(0, 3).map((rec, idx) => (
                  <li key={idx} className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{rec}</li>
                ))}
              </ul>
            </div>
          )}
    ...
    <div className={`rounded-lg shadow-md p-4 sm:p-6 mb-4 ${darkMode ? 'bg-transparent' : 'bg-white'}`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className={`text-xl font-bold ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Clinical Practice Guidelines</h3>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              className={`flex-1 px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm ${
                darkMode
